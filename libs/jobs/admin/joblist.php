...
{		
	...
	$where_query = '';
		...
		{
			$_SESSION['search_admin']['search_type']	=	$_REQUEST['search_type'];
			...
			$_SESSION['search_admin']['keyword']	=	$_REQUEST['keyword'];
			
		}
		else
		{
			unset($_SESSION['search_admin']['search_type']);
			unset($_SESSION['search_admin']['keyword']);
		}	
		
		
		if($_REQUEST['job_status']!='')
		{
				$_SESSION['search_admin']['job_status'] = $_REQUEST['job_status'];
				
				$where_query	.=	" AND j.job_status = '".$_REQUEST['job_status']."'";	
		}
		else
		{
			unset($_SESSION['search_admin']['job_status']);
		...
		if($_REQUEST['txtDateFrom']!='' && $_REQUEST['txtDateTo']!='')
		{
			...
			$date_search_to  =  $date_to[2]."-".$date_to[1]."-".$date_to[0];
				...
				$_SESSION['search_admin']['txtDateFrom'] = $_REQUEST['txtDateFrom'];
				$_SESSION['search_admin']['txtDateTo'] = $_REQUEST['txtDateTo'];
		}
		else
		{
			$date_from = explode("-",$_REQUEST['txtDateFrom']);
...
<table cellpadding="0" cellspacing="0" border="0" width="100%" id="internaltable">
		 <tr>
			<td class="top-bar"><div style=" float:left">Job List</div>
			 <div  class="message"> 
		...
		<tbody><tr class="rowtitle5">
			<td>Total Jobs: <strong><?php echo $total_active_jobs + $total_inactive_jobs;?></strong></td>
			<td>&nbsp;</td>
			<td>Active Jobs: <strong><?php echo $total_active_jobs;?></strong></td>
			<td>&nbsp;</td>
			<td>Inactive Jobs: <strong><?php echo $total_inactive_jobs;?></strong></td>
		<form method="post" action="home.php?p=joblist&mod=jobs"></tr>
		<tr>
			...
			<select name="job_status" class="topsearch">			
                ...
                <option value="1" <?php if($_SESSION['search_admin']['job_status']=='1'){?> selected="selected"<?php }?>>Active</option>	
                <option value="0 <?php if($_SESSION['search_admin']['job_status']=='0'){?> selected="selected"<?php }?>">Suspended </option>
				</select>
                &nbsp;Search:&nbsp;<select name="search_type"  class="topsearch">			
				<option value="job_title" <?php if($_SESSION['search_admin']['search_type']=='job_title'){?> selected="selected"<?php }?>>Job Title</option>
				<option  value="company_name" <?php if($_SESSION['search_admin']['search_type']=='company_name'){?> selected="selected"<?php }?>>Company Name</option> 
			</select>
            
	<input size="25" name="keyword" class="topsearch" value="<?php echo $_SESSION['search_admin']['keyword'];?>" type="text" placeholder = "Keyword">
    Deadline:  
    	<input size="10" readonly="readonly" name="txtDateFrom" id="txtDateFrom" class="topsearchdate" onclick="this.value='';showCalendarControl(document.getElementById('txtDateFrom'));" value="<?php echo $_SESSION['search_admin']['txtDateFrom'];?>" onblur="this.value='';showCalendarControl(document.getElementById('txtDateFrom'));" type="text">&nbsp;To:&nbsp;
					<input size="10" readonly="readonly" name="txtDateTo" id="txtDateTo" class="topsearchdate" onclick="this.value='';showCalendarControl(document.getElementById('txtDateTo'));" onblur="this.value='';showCalendarControl(document.getElementById('txtDateTo'));" value="<?php echo $_SESSION['search_admin']['txtDateTo'];?>" type="text">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	&nbsp;
         ...
         <tr>
					...
					<tr class="rowtitle">
						...
						<?php  
						 $where_search = $_SESSION['admin_search_query'];
								// pagination 2 of 3 
								  $query = "Select count(*) as COUNT from tbl_employer e, tbl_jobs j where j.employer_id = e.id $where_search order by j.id desc";
							    ...
							    $queryfaq="Select e.company_name, e.image, j.* from tbl_employer e, tbl_jobs j where j.employer_id = e.id  $where_search order by j.id desc $limit";
								...
								if(isset($rowfaq))
								{
									...
									{	
										...
										$city_name = city_name($arrfaq['job_country'], $arrfaq['job_state'], $arrfaq['job_city']);
										 ...
										 $query		= mysql_query("Select count(*) as COUNT from tbl_jobs_applied where emp_id = '".$arrfaq['employer_id']."' AND job_id = '".$arrfaq['id']."'" );
										$row_res	= mysql_fetch_array($query);
										$total_applied	= $row_res['COUNT'];
										
						?>
						<tr <?php echo $bgcolor;?>>
							<td style="padding-left:15px;">
								<?php echo $counter;?>							</td>
							<td style="padding-left:10px;"><a href="<?php echo $ru;?>job-detail/<?php echo $arrfaq['id'];?>" target="_blank"><?php  echo stripslashes($arrfaq['job_title']);?></a> <br />
                              <a style="cursor:pointer;" href="home.php?p=apply_jobs_list&mod=jobs&search_type=job_title&keyword=<?php echo stripslashes($arrfaq["job_title"]);?>"><?php echo '('.$total_applied.' member applied)';?></a>
                              </td>
						...
						<td style="padding-left:10px;"><?php  echo stripslashes($arrfaq['expiry_date']); ?></td>
							...
							<?php 
									if($status=="1")
									{
										echo "<a href='".$ruAdmin."home.php?mod=$mod&p=jobsoperations&chstatus=0&faqid=$arrfaq[id]'><img src='img/icon_terms.gif' width='16' height='16' border='0' alt='Inactive'></a>";
									}
									else
									{	
									  echo "<a href='".$ruAdmin."home.php?mod=$mod&p=jobsoperations&chstatus=1&faqid=$arrfaq[id]'><img src='img/icon_terms_2.gif' width='16' height='16' border='0' alt='Active'></a>";
									} 
							 ?>
			...
			<a href="home.php?mod=<?php echo $mod;?>&p=jobsoperations&deletepage=<?php echo $arrfaq[id]?>" onClick="return confirm('Are you sure you want to delete this job?')">
					...
					<?php
						...
						<tr> <td colspan="10" class="msg" align="center"> No Record Found </td>
...
<div align="center">
							...
							if($numrows >$rows_per_page)
							{
								...
								{
									echo " <a href='{$_SERVER['PHP_SELF']}?mod=$mod&p=joblist&pageno=1'>FIRST</a> ";
									$prevpage = $pageno-1;
									echo " <a href='{$_SERVER['PHP_SELF']}?mod=$mod&p=joblist&pageno=$prevpage'>PREV</a> ";
								} 
							//........
							...
							//....................
								...
								{
								$nextpage = $pageno+1;
								echo " <a href='{$_SERVER['PHP_SELF']}?mod=$mod&p=joblist&pageno=$nextpage'>NEXT</a> ";
								echo " <a href='{$_SERVER['PHP_SELF']}?mod=$mod&p=joblist&pageno=$lastpage'>LAST</a> ";
