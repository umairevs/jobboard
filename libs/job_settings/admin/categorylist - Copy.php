...
<table  width="100%" border="0" align="center" cellpadding="0"  cellspacing="0" id="maintable" class="borderclass">
	<tr>
		<td width="100%" valign="top" align="left"><table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td class="top-bar"><div style="float:left">Job Categories</div>
			...
			onClick="window.location='home.php?p=addcategory&mod=<?php echo $mod;?>'" alt="" title="Add New" style="cursor:pointer;" /></div></td>
				</tr>
				<tr>
							...
							<form name="sortings" action="" method="post">
								<?php 
								   if(isset($_SESSION['parent']))
									{
									$querypaging="select count(*) as COUNT from tbl_categories ";
									}
									elseif($filter >= 1)
									{
									$querypaging="select count(*) as COUNT from tbl_categories ";
									...
									{
									$querypaging="select count(*) as COUNT  from tbl_categories  order by  sortorder asc";
									}
								...
								if($numrows==0)
									...
									if(isset($_SESSION['parent']))							   
									{
									$query_paging="select * from tbl_categories  ";
									$querycatlist="select * from tbl_categories  order by sortorder asc $limit";
									}
									elseif($filter >= 1)
									{
									$querycatlist="select * from tbl_categories order by sortorder asc $limit";
									...
									{
									$querycatlist="select * from tbl_categories  order by  sortorder asc $limit";
									$query_paging="select * from tbl_categories order by  sortorder asc";
									}
									//echo $querycatlist;
									$counter=0;
								...
								<tr <?php echo $bgcolor;?>>
									...
									<td align="left" ><?php  echo stripslashes($catlist[category_name]); ?></td>
									<td><input type="text" name="sortorders[]" value="<?php echo stripslashes($catlist[sortorder]);?>" style="width:75px"  />
										<input type="hidden" name="category_ids[]" value="<?php echo stripslashes($catlist[category_id]);?>"  /></td>
									<td><div align="center">
										...
										?>
											<a href="home.php?p=addcategory&mod=<?php echo $mod;?>&editid=<?php echo base64_encode($catlist[category_id]);?>" alt="" title="Edit" /> <img src="img/edit.jpg" width="16" height="16" border="0" alt="Edit"></a> <a href="home.php?p=process&mod=<?php echo $mod;?>&action=del&cId=<?php echo $catlist['category_id']?>" onClick="return confirm('Are you realy want to delete this record?')"> <img src="img/delete.jpg" width="16" height="16" border="0" title="Delete" alt="Delete"> </a> </div></td>
								</tr>
								<?php 
