          ...
          <td colspan="3" align="center" valign="top"><!--home.php?p=categorylist-->
            <table cellpadding="0" cellspacing="0" border="0" width="100%" class="grid" >
              <tr>
                ...
                <?php 
				  ?>
                    <select name="filter" onChange="this.form.submit();" style="width:300px;">
			...
			{
			if($_GET['ctype'] == $rowselopt['category_name'])
			{
			...
			{
			?>
                    <span  id="txtlebel">Move To:</span>
                    <select name="moveto" style="width:300px;">
                      <option value="0"  >TOP</option>
                      <?php $queryselopt="select * from tbl_categories where parent_id=0  order by  sortorder asc";
			$arrselopt=$db->get_results($queryselopt,ARRAY_A);
			if(isset($arrselopt))
			foreach($arrselopt as $rowselopt)
			{
			echo '<option value="'.$rowselopt[category_id].'" >'.$rowselopt[category_name].'&nbsp;&nbsp;</option>';
			echo categorylistmove($rowselopt[category_id]);
			}
			?>
                    </select>
                    <input type="submit" name="movecat" value="Move"  class="txtbutton" style="padding-right:300px;" />
                    <?php	
...
if($numrows==0)
	...
	$i=$rows_per_page*($pageno - 1);
			...
			?>
                      ...
                      <a href="home.php?p=process&mod=<?php echo $mod;?>&action=del&cId=<?php echo $catlist['category_id']?>" onClick="return confirm('Are you realy want to delete this record?')"><img src="img/delete.jpg" width="16" height="16" border="0" title="Delete Parent Category" alt="Delete"> </a> </div></td>
                </tr>
                <?php 
