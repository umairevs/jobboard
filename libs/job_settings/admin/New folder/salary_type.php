<?php
	...
	if($_POST['sort'])
	{
		$count_ids=count($_POST['salary_ids']); 												
		for($i=0; $i<$count_ids; $i++)
		{   
			$cat_ids= trim($_POST['salary_ids'][$i]); 
			$sortvalues= trim($_POST['sortorders'][$i]); 
			$sql_sort=mysql_query("update  tbl_salary_type set sortorder = '".$sortvalues."'	where salary_id=$cat_ids");
			if($sql_sort)
			{					
							...
							<form name="sortings" action="" method="post">
								<?php 
								   if(isset($_SESSION['parent']))
									{
									$querypaging="select count(*) as COUNT from tbl_salary_type ";
									}
									elseif($filter >= 1)
									{
									$querypaging="select count(*) as COUNT from tbl_salary_type ";
									...
									elseif($search!="")
									{
									$query="select count(*) as COUNT  from tbl_salary_type  where salary_title like '%$search%' ";
									}
									else
									{
									$querypaging="select count(*) as COUNT  from tbl_salary_type  order by  sortorder asc";
									}
								...
								if($numrows==0)
									...
									if(isset($_SESSION['parent']))							   
									{
									$query_paging="select * from tbl_salary_type  ";
									$querycatlist="select * from tbl_salary_type  order by sortorder asc $limit";
									}
									elseif($filter >= 1)
									{
									$querycatlist="select * from tbl_salary_type order by sortorder asc $limit";
									...
									{
									$querycatlist="select * from tbl_salary_type  order by  sortorder asc $limit";
									$query_paging="select * from tbl_salary_type order by  sortorder asc";
									}
									//echo $querycatlist;
									$counter=0;
								...
								<tr <?php echo $bgcolor;?>>
									...
									<td align="left" ><?php  echo stripslashes($catlist[salary_title]); ?></td>
									<td><input type="text" name="sortorders[]" value="<?php echo stripslashes($catlist[sortorder]);?>" style="width:75px"  />
										<input type="hidden" name="salary_ids[]" value="<?php echo stripslashes($catlist[salary_id]);?>"  /></td>
									<td><div align="center">
										<?php $status=stripslashes($catlist[status]);
										if($status=="1")
										{
										echo "<a href='home.php?p=process_salary&mod=$mod&chstatus=0&catidchg=$catlist[salary_id]&amp;&filter=$filter'><img src='img/icon_terms.gif' width='16' height='16' border='0' title='Status change' alt='Active'></a>";
										}
										else
										{
										echo "<a href='home.php?p=process_salary&mod=$mod&chstatus=1&catidchg=$catlist[salary_id]&amp;&filter=$filter'><img src='img/icon_terms_2.gif' width='16' height='16' border='0' title='Status change' alt='Inactive'></a>";
										}
										?>
											<a  href="home.php?p=addsalary_type&mod=<?php echo $mod;?>&editid=<?php echo base64_encode($catlist[salary_id]);?>" alt="" title="Edit" /> <img src="img/edit.jpg" width="16" height="16" border="0" alt="Edit"></a> 
                                            <a style="display:none;" href="home.php?p=process_salary&mod=<?php echo $mod;?>&action=del&cId=<?php echo $catlist['salary_id']?>" onClick="return confirm('Are you realy want to delete this record?')"> <img src="img/delete.jpg" width="16" height="16" border="0" title="Delete" alt="Delete"> </a> </div></td>
								</tr>
								<?php 
