...
if(isset($_POST)) 
{
	...
	{
		 $chk_pass_qry ='select password from tbl_job_seaker where 
		  	password  = \''.sha1($old_password).'\' and id="'.$_SESSION['jobseeker']['id'].'" ';
	...
	if($case==1)
	{
		$user_query		=	"Update tbl_job_seaker set password = '".sha1($new_password)."' where  id = '".$_SESSION['jobseeker']['id']."'";	
		$db->query($user_query);
		$_SESSION['success']['profile'] = "Password changed successfully";
		echo "gotonext-SEPARATOR-".SERVER_ROOTPATH."jobseeker-change-password";	
		exit;
	}
